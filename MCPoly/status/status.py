...
def status_judge(s1, s2, converge):
    if s1 == 0 or s1 == 4:
        ...
        print(
            "RMS gradient: {0}             MAX gradient: {0}".format(
                converge[-4], converge[-3]
            )
        )
        print(
            "RMS step: {0}                 MAX step: {0}".format(
...
def multicreatemol(filename, turn, width, height):
    try:
        ...
        raise FileNotFoundError(
            "Perhaps your _trj.xyz file has some errors. Please check the location of your _trj.xyz file and then modify your file."
        )
    mains = ind.readlines()
    num = eval(mains[0][:-1])
...
def thermo(loc, file, keyword):
    ...
    os.chdir(path)
    raise Exception(
        "'{0}.out' may not have keyword freq, or it's aborted. Please check your input file and recalculate it.".format(
            file
        )
    )
...
class status:
    ...
    def converged_energy(self, unit="Eh"):
        ...
        if state != 2:
            raise Exception(
                "This optimisation of '{0}.out' is failed. No converged energy.".format(
                    self.file
                )
            )
        else:
    ...
    def figurecharge(self, width=300, height=300):
        ...
        chargetype = iw.Dropdown(
            description="Atomic Charge Type", options=["Mulliken", "Loewdin"]
        )
        ...
        def GUI(chargetype, num):
            ...
            with output:
                ...
                view3dchoose(
                    self.file, self.loc, choose=[num], width=width, height=height
                )
                print(
                    "Atom: {0}, Element: {1}, Charge: {2}".format(
                        num, alls[num][0], charge(self.loc, self.file, num, chargetype)
                    )
                )
