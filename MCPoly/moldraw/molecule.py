...
def calc_energy(mol, conformerId, minimizeIts):
    ff = AllChem.MMFFGetMoleculeForceField(
        mol, AllChem.MMFFGetMoleculeProperties(mol), confId=conformerId
    )
    ff.Initialize()
    ff.CalcEnergy()
    results = []
...
def cluster_conformers(mol, mode="RMSD", threshold=2.0):
    ...
    rms_clusters = Butina.ClusterData(
        dmat, mol.GetNumConformers(), threshold, isDistData=True, reordering=True
    )
    return rms_clusters
...
def sub_selection(
    substitution, atoms, i, state=1, place=0, strict={1: "X"}, shuffle=0, angle=-1
):
    ...
    if (
        ...
        substitution == "C2H5"
        or substitution == "Et"
        or substitution == "i-C3H7"
        or substitution == "i-Pr"
        or substitution == "iPr"
        or substitution == "t-C4H9"
        or substitution == "t-Bu"
        or substitution == "tBu"
    ):
        ...
        substitution == "C3H7"
        or substitution == "n-C3H7"
        or substitution == "Pr"
        or substitution == "n-Pr"
        or substitution == "nPr"
        or substitution == "C4H9"
        or substitution == "n-C4H9"
        or substitution == "nBu"
        or substitution == "Bu"
        or substitution == "n-Bu"
    ):
        if (
            substitution == "C3H7"
            or substitution == "n-C3H7"
        ...
        substitution == "CF3"
        or substitution == "CCl3"
        or substitution == "CBr3"
        or substitution == "CI3"
    ):
        ...
        substitution == "Py" or substitution == "pyridine" or substitution == "Pyridine"
    ):
        ...
        substitution == "cycloC3H5"
        or substitution == "cycloPr"
        or substitution == "cyclo3"
    ):
        return ring3(atoms, i, state, strict)
    elif (
        substitution == "cycloC5H9"
        or substitution == "cycloPe"
        or substitution == "cyclo5"
    ):
        return ring5(atoms, i, state, strict, shuffle)
    elif (
        substitution == "cycloC6H11"
        or substitution == "cycloHe"
        or substitution == "cyclo6"
    ):
        ...
        substitution == "double5"
        or substitution == "d5"
        or substitution == "D5"
        or substitution == "bicyclo[2.2.1]"
    ):
        return D(5, atoms, i, state, shuffle)
    elif (
        substitution == "double6"
        or substitution == "d6"
        or substitution == "D6"
        or substitution == "bicyclo[2.2.2]"
    ):
...
def Ph(atoms, i, state=1, shuffle=0):
    ...
    atoms.set_dihedral(
        i, total_atoms - 6, total_atoms - 7, total_atoms - 8, 0.5, mask=mask
    )
    mask[-8] = 0
...
def Pyrrole(atoms, i, state=1, place=0, shuffle=0):
    if place == 0:
        ...
        atoms.set_dihedral(
            Cnear, i, total_atoms - 5, total_atoms - 6, -179.5, mask=mask
        )
        mask[-6] = 0
        ...
        mask[-1] = 0
        atoms.set_angle(
            total_atoms - 8, total_atoms - 7, total_atoms - 3, 126, mask=mask
        )
        atoms.set_distance(total_atoms - 3, total_atoms - 7, 1.0800, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms - 8, total_atoms - 7, total_atoms - 3, 179.5, mask=mask
        )
        mask[-3] = 0
        atoms.set_angle(
            total_atoms - 5, total_atoms - 6, total_atoms - 2, 126, mask=mask
        )
        atoms.set_distance(total_atoms - 2, total_atoms - 6, 1.0800, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms - 5, total_atoms - 6, total_atoms - 2, -179.5, mask=mask
        )
        mask[-2] = 0
    elif place == 1:
        atoms.append("N")
        ...
        atoms.set_distance(total_atoms - 7, total_atoms - 8, 1.381, fix=1, mask=mask)
        atoms.set_dihedral(
            Cnear, i, total_atoms - 8, total_atoms - 7, -179.5, mask=mask
        )
        mask[-7] = 0
        ...
        mask[-1] = 0
        atoms.set_angle(
            total_atoms - 5, total_atoms - 6, total_atoms - 3, 126, mask=mask
        )
        atoms.set_distance(total_atoms - 3, total_atoms - 6, 1.0800, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms - 5, total_atoms - 6, total_atoms - 3, 179.5, mask=mask
        )
        mask[-3] = 0
        atoms.set_angle(
            total_atoms - 8, total_atoms - 7, total_atoms - 2, 126, mask=mask
        )
        atoms.set_distance(total_atoms - 2, total_atoms - 7, 1.0800, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms - 8, total_atoms - 7, total_atoms - 2, -179.5, mask=mask
        )
        mask[-2] = 0
    elif place == 2:
        atoms.append("C")
    ...
    for j in range(2):
        ...
        if ring_element == "O":
            ...
            if j == 0:
                for l in range(10):
                    atoms.set_distance(
                        total_atoms - 6, total_atoms - 7, 1.4100, mask=mask
                    )
                ...
                for l in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms - 7, total_atoms - 6, 1.4100, mask=mask
                    )
        elif ring_element == "S":
            elements[-7 + j] = "S"
            atoms.set_chemical_symbols(elements)
            ...
            if j == 0:
                for l in range(10):
                    atoms.set_distance(
                        total_atoms - 6, total_atoms - 7, 1.8100, mask=mask
                    )
                ...
                for l in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms - 7, total_atoms - 6, 1.8100, mask=mask
                    )
        elif ring_element == "N":
            elements[-7 + j] = "N"
            atoms.set_chemical_symbols(elements)
            ...
            if j == 0:
                for l in range(10):
                    atoms.set_distance(
                        total_atoms - 6, total_atoms - 7, 1.4480, mask=mask
                    )
                ...
                for l in range(10):
                    atoms.set_angle(
                        i, total_atoms - 7, total_atoms - 3, 109.5, mask=mask
                    )
                    atoms.set_angle(
                        total_atoms - 6,
                        total_atoms - 7,
                ...
                atoms.set_distance(
                    total_atoms - 3, total_atoms - 7, 1.0100, fix=1, mask=mask
                )
                mask[-3] = 0
            elif j == 1:
                for l in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms - 7, total_atoms - 6, 1.4480, mask=mask
                    )
                mask[-6] = 0
                mask[-1] = 1
                for l in range(10):
                    atoms.set_angle(
                        i, total_atoms - 6, total_atoms - 1, 109.5, mask=mask
                    )
                    atoms.set_angle(
                        total_atoms - 7,
                        total_atoms - 6,
                ...
                atoms.set_distance(
                    total_atoms - 1, total_atoms - 6, 1.0100, fix=1, mask=mask
                )
                mask[-1] = 0
        mask[-7 + j] = 0
    try:
...
def ring5(atoms, i, state=1, strict={1: "X"}, shuffle=0):
    ...
    if strict[1] == "N":
        ...
        atoms.set_distance(
            total_atoms - 10, total_atoms + box[0], 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[0], total_atoms - 10, 164.01, mask=mask
        )
        mask[-10] = 0
        atoms.set_angle(i, total_atoms + box[1], total_atoms - 9, 104.98, mask=mask)
        atoms.set_distance(
            total_atoms - 9, total_atoms + box[1], 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[1], total_atoms - 9, -164.01, mask=mask
        )
        mask[-9] = 0
    else:
        atoms.set_positions(x)
        ...
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[0], total_atoms - 10, -161, mask=mask
        )
        mask[-10] = 0
        ...
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[1], total_atoms - 9, 161, mask=mask
        )
        mask[-9] = 0
    ...
    if strict[1] == "N":
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 10, total_atoms - 4, 109.5, mask=mask
        )
        ...
        mask[-4] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 10, total_atoms - 3, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 3, total_atoms - 10, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 10, total_atoms - 3, 95, mask=mask
        )
        mask[-3] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 9, total_atoms - 2, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 2, total_atoms - 9, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 9, total_atoms - 2, 140.5, mask=mask
        )
        mask[-2] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 9, total_atoms - 1, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 1, total_atoms - 9, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 9, total_atoms - 1, -95, mask=mask
        )
        mask[-1] = 0
    else:
        atoms.set_angle(
        ...
        atoms.set_distance(total_atoms - 4, total_atoms - 10, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 10, total_atoms - 4, -90.5, mask=mask
        )
        mask[-4] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 10, total_atoms - 3, 109.5, mask=mask
        ...
        atoms.set_distance(total_atoms - 3, total_atoms - 10, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 10, total_atoms - 3, 145, mask=mask
        )
        mask[-3] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 9, total_atoms - 2, 109.5, mask=mask
        ...
        atoms.set_distance(total_atoms - 2, total_atoms - 9, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 9, total_atoms - 2, 90.5, mask=mask
        )
        mask[-2] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 9, total_atoms - 1, 109.5, mask=mask
        ...
        atoms.set_distance(total_atoms - 1, total_atoms - 9, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 9, total_atoms - 1, -145, mask=mask
        )
        mask[-1] = 0
    l = 0
    for j in range(4):
        ...
        if ring_element == "O":
            ...
            if j + 2 == 2:
                atoms.set_distance(
                    total_atoms + l, total_atoms + box[0], 0, fix=1, mask=mask
                )
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 10, 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(total_atoms + l, i, 1.4100, fix=1, mask=mask)
            elif j + 2 == 3:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 10, 0, fix=1, mask=mask
                )
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[0], 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 9, 1.4100, fix=1, mask=mask
                    )
            elif j + 2 == 4:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 9, 0, fix=1, mask=mask
                )
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[1], 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 10, 1.4100, fix=1, mask=mask
                    )
                ...
                atoms.set_distance(
                    total_atoms + l, total_atoms + box[1], 0, fix=1, mask=mask
                )
                for k in range(10):
                    atoms.set_distance(total_atoms + l, i, 1.4100, fix=1, mask=mask)
                    atoms.set_distance(
            ...
            if j + 2 == 4:
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[1], 1.8100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 10, 1.8100, fix=1, mask=mask
                    )
            elif j + 2 == 3:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 10, 0, fix=1, mask=mask
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[0], 1.8100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 9, 1.8100, fix=1, mask=mask
                    )
            elif j + 2 == 5:
                atoms.set_distance(
                    total_atoms + l, total_atoms + box[1], 0, fix=1, mask=mask
            ...
            if j + 2 == 4:
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[1], 1.4480, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 10, 1.4480, fix=1, mask=mask
                    )
                mask[-2] = 0
                for k in range(10):
                    atoms.set_angle(
                ...
                atoms.set_distance(
                    total_atoms + l + 1, total_atoms + l, 1.0100, fix=1, mask=mask
                )
            elif j + 2 == 3:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 10, 0, fix=1, mask=mask
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms + box[0], 1.4480, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 9, 1.4480, fix=1, mask=mask
                    )
                mask[-2] = 0
                for k in range(10):
                    atoms.set_angle(
                ...
                for k in range(10):
                    atoms.set_angle(
                        i, total_atoms + l, total_atoms + l + 1, 109.5, mask=mask
                    )
                    atoms.set_angle(
                        total_atoms - 9,
                        total_atoms + l,
...
def ring6(atoms, i, state=1, strict={1: "X"}, shuffle=0):
    ...
    if strict[1] == "N":
        ...
        atoms.set_distance(
            total_atoms - 13, total_atoms + box[0], 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[0], total_atoms - 13, 170, mask=mask
        )
        mask[-13] = 0
        atoms.set_angle(i, total_atoms + box[1], total_atoms - 12, 115.7, mask=mask)
        atoms.set_distance(
            total_atoms - 12, total_atoms + box[1], 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[1], total_atoms - 12, -170, mask=mask
        )
        mask[-12] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 13, total_atoms - 11, 113.7, mask=mask
        )
        for k in range(10):
            atoms.set_distance(
                total_atoms - 11, total_atoms - 12, 1.5290, fix=1, mask=mask
            )
            atoms.set_distance(
                total_atoms - 11, total_atoms - 13, 1.5290, fix=1, mask=mask
            )
        mask[-11] = 0
    else:
        ...
        mask[-12] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 13, total_atoms - 11, 107.4, mask=mask
        )
        atoms.set_distance(total_atoms - 11, total_atoms - 13, 1.5290, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 13, total_atoms - 11, 55, mask=mask
        )
        mask[-11] = 0
    ...
    if strict[1] == "N":
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 13, total_atoms - 6, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 6, total_atoms - 13, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 13, total_atoms - 6, 165, mask=mask
        )
        mask[-6] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 13, total_atoms - 5, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 5, total_atoms - 13, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 13, total_atoms - 5, 59.5, mask=mask
        )
        mask[-5] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 12, total_atoms - 4, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 4, total_atoms - 12, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 12, total_atoms - 4, -165, mask=mask
        )
        mask[-4] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 12, total_atoms - 3, 109.5, mask=mask
        )
        atoms.set_distance(total_atoms - 3, total_atoms - 12, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 12, total_atoms - 3, -59.5, mask=mask
        )
        mask[-3] = 0
    else:
        atoms.set_angle(
        ...
        atoms.set_distance(total_atoms - 6, total_atoms - 13, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 13, total_atoms - 6, -75, mask=mask
        )
        mask[-6] = 0
        atoms.set_angle(
            total_atoms + box[0], total_atoms - 13, total_atoms - 5, 109.5, mask=mask
        ...
        atoms.set_distance(total_atoms - 5, total_atoms - 13, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[0], total_atoms - 13, total_atoms - 5, 179.5, mask=mask
        )
        mask[-5] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 12, total_atoms - 4, 109.5, mask=mask
        ...
        atoms.set_distance(total_atoms - 4, total_atoms - 12, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            i, total_atoms + box[1], total_atoms - 12, total_atoms - 4, 75, mask=mask
        )
        mask[-4] = 0
        atoms.set_angle(
            total_atoms + box[1], total_atoms - 12, total_atoms - 3, 109.5, mask=mask
        ...
        mask[-3] = 0
    atoms.set_angle(
        total_atoms - 13, total_atoms - 11, total_atoms - 2, 109.5, mask=mask
    )
    ...
    mask[-2] = 0
    atoms.set_angle(
        total_atoms - 13, total_atoms - 11, total_atoms - 1, 109.5, mask=mask
    )
    ...
    for j in range(5):
        ...
        if ring_element == "O":
            ...
            if j + 2 == 4:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 11, 0.1, fix=1, mask=mask
                )
                atoms.set_angle(
                    total_atoms + box[0],
                    total_atoms - 13,
                ...
                atoms.set_distance(
                    total_atoms + l, total_atoms - 13, 1.4100, fix=1, mask=mask
                )
                atoms.set_dihedral(
                    i,
                    total_atoms + box[0],
                ...
                atoms.set_distance(
                    total_atoms + l, total_atoms - 13, 0, fix=1, mask=mask
                )
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 11, 1.4100, fix=1, mask=mask
                    )
            elif j + 2 == 5:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 12, 0, fix=1, mask=mask
                )
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 12, 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(total_atoms + l, i, 1.4100, fix=1, mask=mask)
            l = l + 1
        elif ring_element == "S":
            ...
            if j + 2 == 4:
                ...
                atoms.set_distance(
                    total_atoms + l, total_atoms - 13, 1.8100, fix=1, mask=mask
                )
                atoms.set_dihedral(
                    i,
                    total_atoms + box[0],
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 11, 1.8100, fix=1, mask=mask
                    )
            elif j + 2 == 5:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 12, 0, fix=1, mask=mask
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 12, 1.8100, fix=1, mask=mask
                    )
                    atoms.set_distance(total_atoms + l, i, 1.8100, fix=1, mask=mask)
            l = l + 1
        elif ring_element == "N":
            ...
            if j + 2 == 4:
                atoms.set_distance(
                    total_atoms + l, total_atoms - 11, 0, fix=1, mask=mask
                )
                atoms.set_distance(total_atoms + l, i, 5, mask=mask)
                # Pseudo-Optimization
                for k in range(10):
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 13, 1.4480, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 12, 1.4480, fix=1, mask=mask
                    )
                mask[-2] = 0
                atoms.set_angle(
                    total_atoms - 13,
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms + l, total_atoms - 11, 1.4480, fix=1, mask=mask
                    )
                mask[-2] = 0
                for k in range(10):
                    atoms.set_angle(
...
def D(member, atoms, i, state=1, shuffle=0):
    ...
    mask[-10] = 0
    atoms.set_angle(
        total_atoms + box[0], total_atoms - 11, total_atoms - 9, 110.5, mask=mask
    )
    atoms.set_dihedral(
        i, total_atoms + box[0], total_atoms - 11, total_atoms - 9, 290, mask=mask
    )
    for k in range(10):
    ...
    mask[-6] = 0
    atoms.set_angle(
        total_atoms + box[0], total_atoms - 11, total_atoms - 5, 109.5, mask=mask
    )
    atoms.set_distance(total_atoms - 5, total_atoms - 11, 1.0900, fix=1, mask=mask)
    atoms.set_dihedral(
        i, total_atoms + box[0], total_atoms - 11, total_atoms - 5, 179.5, mask=mask
    )
    mask[-5] = 0
    atoms.set_angle(
        total_atoms + box[1], total_atoms - 10, total_atoms - 4, 109.5, mask=mask
    )
    atoms.set_distance(total_atoms - 4, total_atoms - 10, 1.0900, fix=1, mask=mask)
    atoms.set_dihedral(
        i, total_atoms + box[1], total_atoms - 10, total_atoms - 4, 75, mask=mask
    )
    mask[-4] = 0
    atoms.set_angle(
        total_atoms + box[1], total_atoms - 10, total_atoms - 3, 109.5, mask=mask
    )
    atoms.set_distance(total_atoms - 3, total_atoms - 10, 1.0900, fix=1, mask=mask)
    atoms.set_dihedral(
        i, total_atoms + box[1], total_atoms - 10, total_atoms - 3, -179.5, mask=mask
    )
    mask[-3] = 0
    atoms.set_angle(
        total_atoms - 11, total_atoms - 9, total_atoms - 2, 109.5, mask=mask
    )
    atoms.set_distance(total_atoms - 2, total_atoms - 9, 1.0900, fix=1, mask=mask)
    atoms.set_dihedral(
        total_atoms + box[0],
    ...
    mask[-2] = 0
    atoms.set_angle(
        total_atoms - 11, total_atoms - 9, total_atoms - 1, 109.5, mask=mask
    )
    atoms.set_distance(total_atoms - 1, total_atoms - 9, 1.0900, fix=1, mask=mask)
    atoms.set_dihedral(
        total_atoms + box[0],
    ...
    if member == 5:
        ...
        atoms.set_distance(
            total_atoms - 3, total_atoms + box[1] - 3, 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[1] - 3, total_atoms - 3, 60, mask=mask
        )
        for k in range(10):
            atoms.set_distance(
                total_atoms - 3, total_atoms + box[1] - 3, 1.5290, fix=1, mask=mask
            ...
            atoms.set_distance(
                total_atoms - 3, total_atoms - 14, 1.5290, fix=1, mask=mask
            )
        mask[-3] = 0
        atoms.set_angle(
            total_atoms + box[1] - 3, total_atoms - 3, total_atoms - 2, 113.2, mask=mask
        )
        ...
        mask[-2] = 0
        atoms.set_angle(
            total_atoms + box[1] - 3, total_atoms - 3, total_atoms - 1, 113.2, mask=mask
        )
        ...
        atoms.set_distance(
            total_atoms - 6, total_atoms + box[1] - 6, 1.5290, fix=1, mask=mask
        )
        atoms.set_dihedral(
            Cnear, i, total_atoms + box[1] - 6, total_atoms - 6, 67.5, mask=mask
        )
        mask[-6] = 0
        atoms.set_angle(
            total_atoms + box[0] - 6,
        ...
        for k in range(10):
            atoms.set_distance(
                total_atoms - 5, total_atoms - 6, 1.5290, fix=1, mask=mask
            )
            atoms.set_distance(
                total_atoms - 5, total_atoms - 17, 1.5290, fix=1, mask=mask
            )
        mask[-5] = 0
        atoms.set_angle(
            total_atoms + box[1] - 6, total_atoms - 6, total_atoms - 4, 110.2, mask=mask
        )
        ...
        mask[-4] = 0
        atoms.set_angle(
            total_atoms + box[1] - 6, total_atoms - 6, total_atoms - 3, 110.2, mask=mask
        )
        ...
        mask[-3] = 0
        atoms.set_angle(
            total_atoms - 17, total_atoms - 5, total_atoms - 2, 110.2, mask=mask
        )
        ...
        mask[-2] = 0
        atoms.set_angle(
            total_atoms - 17, total_atoms - 5, total_atoms - 1, 110.2, mask=mask
        )
...
def Bpin(atoms, i, state=1, shuffle=0):
    ...
    x[-1][2] = x[-1][2] + 50
    atoms.set_dihedral(
        i, total_atoms - 8, total_atoms - 6, total_atoms - 4, 120, mask=mask
    )
    ...
    mask[-4] = 0
    atoms.set_dihedral(
        i, total_atoms - 8, total_atoms - 6, total_atoms - 3, 240, mask=mask
    )
    ...
    mask[-3] = 0
    atoms.set_dihedral(
        i, total_atoms - 7, total_atoms - 5, total_atoms - 2, 120, mask=mask
    )
    ...
    mask[-2] = 0
    atoms.set_dihedral(
        i, total_atoms - 7, total_atoms - 5, total_atoms - 1, 240, mask=mask
    )
    ...
    if (
        atoms.get_distance(total_atoms - 4, total_atoms - 5) <= 1.3
        or atoms.get_distance(total_atoms - 3, total_atoms - 5) <= 1.3
        or atoms.get_distance(total_atoms - 2, total_atoms - 6) <= 1.3
        or atoms.get_distance(total_atoms - 1, total_atoms - 6) <= 1.3
    ):
        mask = [0] * (total_atoms - 4) + [1] * 4
        x = atoms.get_positions()
        x[-4][0] = x[-4][0] - 50
        ...
        x[-1][2] = x[-1][2] + 50
        atoms.set_dihedral(
            i, total_atoms - 8, total_atoms - 6, total_atoms - 4, 300, mask=mask
        )
        atoms.set_angle(
            total_atoms - 8, total_atoms - 6, total_atoms - 4, 111.5, mask=mask
        )
        atoms.set_distance(total_atoms - 4, total_atoms - 6, 1.0900, fix=1)
        mask[-4] = 0
        atoms.set_dihedral(
            i, total_atoms - 8, total_atoms - 6, total_atoms - 3, 179.5, mask=mask
        )
        atoms.set_angle(
            total_atoms - 8, total_atoms - 6, total_atoms - 3, 111.5, mask=mask
        )
        atoms.set_distance(total_atoms - 3, total_atoms - 6, 1.0900, fix=1)
        mask[-3] = 0
        atoms.set_dihedral(
            i, total_atoms - 7, total_atoms - 5, total_atoms - 2, 60, mask=mask
        )
        atoms.set_angle(
            total_atoms - 7, total_atoms - 5, total_atoms - 2, 111.5, mask=mask
        )
        atoms.set_distance(total_atoms - 2, total_atoms - 5, 1.0900, fix=1)
        mask[-2] = 0
        atoms.set_dihedral(
            i, total_atoms - 7, total_atoms - 5, total_atoms - 1, 179.5, mask=mask
        )
        atoms.set_angle(
            total_atoms - 7, total_atoms - 5, total_atoms - 1, 111.5, mask=mask
        )
        atoms.set_distance(total_atoms - 1, total_atoms - 5, 1.0900, fix=1)
    return atoms
...
def near(i, atoms, target="ALL", ignore=[], limit=True, reverse=False):
    ...
    if target == "all" or target == "All" or target == "ALL":
        ...
        targeted_distances = np.delete(
            targeted_distances, np.where(targeted_distances == 0.0)
        )
    ...
    for distance in distances:
        ...
        if reverse == True:
            ...
            if (
                abs(
                    min(targeted_distances)
                    - atoms.get_distances(i, total_atoms + _near)
                )
                <= 0.001
            ):
...
def bind3(atoms, bond, state=[1, 1], strict={1: "X"}):
    ...
    if strict[1] == "O":
        ...
        atoms.set_distance(total_atoms - 1, total_atoms - 3, 1.0900, fix=1, mask=mask)
        atoms.set_dihedral(
            total_atoms - 1, total_atoms - 3, i1, Cnear, -70.5, mask=mask
        )
        for k in range(10):
...
def bind5(
    ...
    if side < 2:
        ...
        while 1:
            ...
            atoms.set_distance(
                total_atoms - 2, total_atoms - 3, 1.0900, fix=1, mask=mask
            )
            if (
                atoms.get_distance(total_atoms - 2, Hnear1) > 1.4
                and atoms.get_distance(total_atoms - 2, Hnear2) > 1.4
            ):
                if (
                    abs(
                        atoms.get_angle(Hnear1, total_atoms - 3, total_atoms - 2)
        ...
        atoms.set_dihedral(
            ...
            Hnear1,
            i1,
            atoms.get_dihedral(total_atoms - 1, total_atoms - 3, Hnear1, i1) + 110,
            mask=mask,
        )
        xoy = near(total_atoms - 1, atoms, limit=False)
        if atoms.get_distance(xoy, total_atoms - 1) <= 0.9:
            atoms.set_dihedral(
                ...
                Hnear1,
                i1,
                atoms.get_dihedral(total_atoms - 1, total_atoms - 3, Hnear1, i1) - 220,
                mask=mask,
            )
    else:
        atoms.set_distance(total_atoms - 2, total_atoms - 3, 0, fix=1, mask=mask)
    ...
    for i in range(len(needelements)):
        ...
        if places[i] == 2:
            ...
            if needelements[i] == "N":
                for k in range(10):
                    ...
                    atoms.set_distance(
                        Hnear1, total_atoms - 3, 1.4480, fix=1, mask=mask
                    )
                mask[Hnear1] = 0
                total_atoms = len(atoms)
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        Hnear1, total_atoms - 3, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
                    CnearH = near(Hnear1, atoms, "H", limit=False)
        ...
        if places[i] == 3:
            ...
            if needelements[i] == "N":
                for k in range(10):
                    atoms.set_distance(
                        total_atoms - 3, Hnear1, 1.4480, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms - 3, Hnear2, 1.4480, fix=1, mask=mask
                    )
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms - 3, Hnear1, 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms - 3, Hnear2, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
        ...
        if places[i] == 4:
            ...
            if needelements[i] == "N":
                for k in range(10):
                    ...
                    atoms.set_distance(
                        Hnear2, total_atoms - 3, 1.4480, fix=1, mask=mask
                    )
                mask[Hnear2] = 0
                total_atoms = len(atoms)
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        Hnear2, total_atoms - 3, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
                    CnearH = near(Hnear2, atoms, "H", limit=False)
...
def bind6(atoms, bond, state=[1, 1], strict={1: "X"}, side=0, shuffle=0, less5=False):
    ...
    for k in range(10):
        ...
        if (
            less5 == True
            and atoms.get_dihedral(Hnear2, i2, i1, Hnear1) > 10
            and atoms.get_dihedral(Hnear2, i2, i1, Hnear1) < 350
        ):
            if (
                atoms.get_dihedral(i1, i2, Hnear2, total_atoms - 5) <= 30
                and atoms.get_dihedral(i1, i2, Hnear2, total_atoms - 5) >= 0
            ):
                atoms.set_dihedral(i1, i2, Hnear2, total_atoms - 5, 47.9, mask=mask)
            if (
                atoms.get_dihedral(i1, i2, Hnear2, total_atoms - 5) >= 330
    ...
    for k in range(10):
        ...
        atoms.set_angle(
            total_atoms - 5, total_atoms - 6, total_atoms - 4, 111.5, mask=mask
        )
    mask[-4] = 0
    ...
    atoms.set_dihedral(
        ...
        Hnear1,
        i1,
        atoms.get_dihedral(total_atoms - 3, total_atoms - 6, Hnear1, i1) - 110,
        mask=mask,
    )
    ...
    if atoms.get_distance(xoy, total_atoms - 3) <= 0.9:
        atoms.set_dihedral(
            ...
            Hnear1,
            i1,
            atoms.get_dihedral(total_atoms - 3, total_atoms - 6, Hnear1, i1) + 220,
            mask=mask,
        )
    mask[-3] = 0
    ...
    for k in range(10):
        ...
        atoms.set_angle(
            total_atoms - 6, total_atoms - 5, total_atoms - 2, 111.5, mask=mask
        )
    mask[-2] = 0
    atoms.set_distance(total_atoms - 1, total_atoms - 2, 0, fix=1, mask=mask)
    atoms.set_dihedral(
        ...
        Hnear2,
        i2,
        atoms.get_dihedral(total_atoms - 1, total_atoms - 5, Hnear2, i2) - 110,
        mask=mask,
    )
    xoy = near(total_atoms - 1, atoms, limit=False)
    if atoms.get_distance(xoy, total_atoms - 1) <= 0.9:
        atoms.set_dihedral(
            ...
            Hnear2,
            i2,
            atoms.get_dihedral(total_atoms - 1, total_atoms - 5, Hnear2, i2) + 220,
            mask=mask,
        )
    ...
    for i in range(len(needelements)):
            ...
            if needelements[i] == "N":
                for k in range(10):
                    if state[0] == 1:
                        ...
                        atoms.set_distance(Hnear1, i1, 1.3400, fix=1, mask=mask)
                    atoms.set_distance(
                        Hnear1, total_atoms - 6, 1.4480, fix=1, mask=mask
                    )
                mask[Hnear1] = 0
                total_atoms = len(atoms)
                CnearH = near(Hnear1, atoms, "H", limit=False)
                ...
                for k in range(10):
                    if state[0] == 1:
                        ...
                        atoms.set_distance(Hnear1, i1, 1.3640, fix=1, mask=mask)
                    atoms.set_distance(
                        Hnear1, total_atoms - 6, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
                    CnearH = near(Hnear1, atoms, "H", limit=False)
        ...
        if places[i] == 3:
            ...
            if needelements[i] == "N":
                for k in range(10):
                    atoms.set_distance(
                        total_atoms - 6 + error, Hnear1, 1.4480, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms - 6 + error,
                        total_atoms - 5 + error,
                ...
                mask[CnearH] = 1
                atoms.set_dihedral(
                    i1, Hnear1, total_atoms - 6 + error, CnearH, -150, mask=mask
                )
                for k in range(10):
                    atoms.set_angle(
                        Hnear1, total_atoms - 6 + error, CnearH, 109.5, mask=mask
                    )
                    atoms.set_angle(
                        total_atoms - 5 + error,
                        total_atoms - 6 + error,
                ...
                for k in range(10):
                    atoms.set_distance(
                        total_atoms - 6 + error, Hnear1, 1.4100, fix=1, mask=mask
                    )
                    atoms.set_distance(
                        total_atoms - 6 + error,
                        total_atoms - 5 - error,
        ...
        if places[i] == 4:
            ...
            if needelements[i] == "N":
                for k in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms - 5 + error, Hnear2, 1.4480, fix=1, mask=mask
                    )
                ...
                mask[CnearH] = 1
                atoms.set_dihedral(
                    i2, Hnear2, total_atoms - 5 + error, CnearH, -150, mask=mask
                )
                for k in range(10):
                    atoms.set_angle(
                        Hnear2, total_atoms - 5 + error, CnearH, 109.5, mask=mask
                    )
                    atoms.set_angle(
                        total_atoms - 6 + error,
                        total_atoms - 5 + error,
                ...
                for k in range(10):
                    ...
                    atoms.set_distance(
                        total_atoms - 5 + error, Hnear2, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
            ...
            if needelements[i] == "N":
                for k in range(10):
                    if state[0] == 1:
                        ...
                        atoms.set_distance(Hnear2, i2, 1.3400, fix=1, mask=mask)
                    atoms.set_distance(
                        Hnear2, total_atoms - 5 + error, 1.4480, fix=1, mask=mask
                    )
                mask[Hnear2] = 0
                total_atoms = len(atoms)
                CnearH = near(Hnear2, atoms, "H", limit=False)
                ...
                for k in range(10):
                    atoms.set_angle(i2, Hnear2, CnearH, 109.5, mask=mask)
                    atoms.set_angle(
                        total_atoms - 5 + error, Hnear2, CnearH, 109.5, mask=mask
                    )
                atoms.set_dihedral(i1, i2, Hnear2, CnearH, -150, mask=mask)
                atoms.set_distance(CnearH, Hnear2, 1.0100, fix=1)
            elif needelements[i] == "O":
                for k in range(10):
                    if state[0] == 1:
                        ...
                        atoms.set_distance(Hnear2, i2, 1.3640, fix=1, mask=mask)
                    atoms.set_distance(
                        Hnear2, total_atoms - 5 + error, 1.4100, fix=1, mask=mask
                    )
                for j in range(2):
                    total_atoms = len(atoms)
                    CnearH = near(Hnear2, atoms, "H", limit=False)
...
def bindconj5(atoms, bond, state=[1, 1], strict={1: "X"}):
    ...
    if place == 2:
        ...
        if element == "N":
            for k in range(10):
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 6, 1.3810, fix=1, mask=mask
                )
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 5, 1.3670, fix=1, mask=mask
                )
            mask[-4] = 0
            ...
            for k in range(10):
                atoms.set_angle(
                    total_atoms - 4, total_atoms - 6, total_atoms - 1, 124.5, mask=mask
                )
            ...
            for k in range(10):
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 6, 1.3600, fix=1, mask=mask
                )
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 5, 1.3670, fix=1, mask=mask
                )
        ...
        atoms.set_dihedral(
            total_atoms - 3, total_atoms - 4, total_atoms - 5, i2, 0.5, mask=mask
        )
        for k in range(10):
            atoms.set_angle(
                total_atoms - 6, total_atoms - 4, total_atoms - 3, 126.5, mask=mask
            )
            atoms.set_angle(
                total_atoms - 5, total_atoms - 4, total_atoms - 3, 126.5, mask=mask
            )
        mask[-3] = 0
        ...
        for k in range(10):
            ...
            atoms.set_angle(
                total_atoms - 4, total_atoms - 5, total_atoms - 2, 126.5, mask=mask
            )
        mask[-2] = 0
    elif place == 3:
        ...
        if element == "N":
            for k in range(10):
                ...
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 5, 1.3810, fix=1, mask=mask
                )
            mask[-4] = 0
            atoms.set_distance(total_atoms - 1, total_atoms - 4, 0, fix=1, mask=mask)
            atoms.set_distance(
                total_atoms - 1, total_atoms - 4, 1.0100, fix=1, mask=mask
            )
            atoms.set_dihedral(
                i1, total_atoms - 6, total_atoms - 4, total_atoms - 1, 110, mask=mask
            )
            for k in range(10):
                atoms.set_angle(
                    total_atoms - 6, total_atoms - 4, total_atoms - 1, 124.5, mask=mask
                )
                atoms.set_angle(
                    total_atoms - 5, total_atoms - 4, total_atoms - 1, 124.5, mask=mask
                )
            mask[-1] = 0
        elif element == "O":
            for k in range(10):
                ...
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 5, 1.3600, fix=1, mask=mask
                )
            mask[-4] = 0
        ...
        for k in range(10):
            ...
            atoms.set_angle(
                total_atoms - 4, total_atoms - 6, total_atoms - 3, 126.5, mask=mask
            )
        mask[-3] = 0
        atoms.set_distance(total_atoms - 2, total_atoms - 5, 1.0100, fix=1, mask=mask)
        atoms.set_dihedral(total_atoms - 2, total_atoms - 5, i2, i1, 0.5, mask=mask)
        ...
        if element == "N":
            for k in range(10):
                ...
                atoms.set_distance(
                    total_atoms - 4, total_atoms - 6, 1.3670, fix=1, mask=mask
                )
            mask[-4] = 0
            # atoms.set_distance(total_atoms-1,total_atoms-6,0,fix=1,mask=mask)
            atoms.set_dihedral(
            ...
            for k in range(10):
                atoms.set_angle(
                    total_atoms - 4, total_atoms - 5, total_atoms - 1, 124.5, mask=mask
                )
                atoms.set_angle(i2, total_atoms - 5, total_atoms - 1, 124.5, mask=mask)
            atoms.set_distance(
                total_atoms - 1, total_atoms - 5, 1.0100, fix=1, mask=mask
            )
            mask[-1] = 0
        elif element == "O":
            for k in range(10):
        ...
        for k in range(10):
            ...
            atoms.set_angle(
                total_atoms - 4, total_atoms - 6, total_atoms - 2, 126.5, mask=mask
            )
        mask[-2] = 0
    if element != "N":
        del atoms[-1]
...
def bindconj6(atoms, bond, state=[1, 1], strict={1: "X"}):
    ...
    atoms.set_dihedral(
        total_atoms - 2, total_atoms - 6, total_atoms - 8, i1, 179.5, mask=mask
    )
    atoms.set_distance(total_atoms - 2, total_atoms - 6, 1.0800, fix=1, mask=mask)
    mask[-2] = 0
...
def CNH(atoms, Cnear, state=[1, 1]):
    ...
    atoms.set_distance(total_atoms - 1, total_atoms - 2, 1.0100, fix=1, mask=mask)
    atoms.set_dihedral(
        Cnear1, Cnear, total_atoms - 2, total_atoms - 1, 179.5, mask=mask
    )
    for i in range(2):
        Hnear = near(Cnear, atoms, "H", limit=False)
        del atoms[Hnear]
...
def N(atoms, Cnear, state=[1, 1]):
    ...
    for k in range(10):
        ...
        atoms.set_dihedral(
            CnearC3, CnearC1, total_atoms - 2, total_atoms - 1, 150, mask=mask
        )
...
class molecule:
    ...
    def sub(
        self, substitution, i, state=1, place=0, strict={1: "X"}, shuffle=0, angle=-1
    ):
        """
        ...
        """
        self.atoms = sub_selection(
            substitution, self.atoms, i, state, place, strict, shuffle, angle
        )
        return self.atoms
    ...
    def straight(self, start, end):
        ...
        Bpos = pos[end]
        self.atoms.rotate(
            m.atan((Bpos[-1] - Apos[-1]) / (Bpos[1] - Apos[1])) / m.pi * 180, "x", Apos
        )
        pos = self.atoms.get_positions()
        Apos = pos[start]
        Bpos = pos[end]
        self.atoms.rotate(
            m.atan((Bpos[-1] - Apos[-1]) / (Bpos[0] - Apos[0])) / m.pi * 180, "y", Apos
        )
        pos = self.atoms.get_positions()
        Apos = pos[start]
        Bpos = pos[end]
        self.atoms.rotate(
            m.atan(-(Bpos[1] - Apos[1]) / (Bpos[0] - Apos[0])) / m.pi * 180 + 180,
            "z",
            Apos,
        )
        pos = self.atoms.get_positions()
