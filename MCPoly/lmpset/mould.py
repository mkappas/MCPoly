...
def replication(
    filename, x, xgap, y, ygap, z, zgap, absolute=False, startloc=[0, 0, 0], loc="./"
):
    if absolute == True:
    ...
    for line in inp:
        ...
        if a1:
            ...
            f.write(
                "{0:.5f} {1:.5f} xlo xhi\n".format(
                    xloc, xloc + (xd - real[0]) * x + xgap * (x - 1)
                )
            )
            if absolute == True:
                real[0] = xd
            ...
            f.write(
                "{0:.5f} {1:.5f} ylo yhi\n".format(
                    yloc, yloc + (yd - real[1]) * y + ygap * (y - 1)
                )
            )
            if absolute == True:
                real[1] = yd
            ...
            f.write(
                "{0:.5f} {1:.5f} zlo zhi\n".format(
                    zloc, zloc + (zd - real[2]) * z + zgap * (z - 1)
                )
            )
            if absolute == True:
                real[2] = zd
...
class mould:
    ...
    def cube(self, x, xgap, y, ygap, z, zgap, absolute=False, startloc=[0, 0, 0]):
        ...
        * * * * *     Cube Structure
        ...
        """
        return replication(
            self.atoms, x, xgap, y, ygap, z, zgap, absolute, startloc, self.loc
        )
