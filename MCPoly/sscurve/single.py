...
def curvef(loc, polymer, start, end, savefig, savedata, xx=False):
    ...
    for data in res:
        ...
        f.write(
            "{0:.5f},{1}\n".format(
                atoms.get_distance(start, end) / x, strain.group(0)[1:]
            )
        )
    ...
    if xx == False:
        a = sns.scatterplot(
            data=datum,
            x=datum["Strain Length(%)"],
            y=datum["Stress Force(nN)"],
            color="black",
            alpha=0.5,
        )
    ...
    def autocurve(self, savefig=True, savedata=True, xx=False):
        ...
        if t == 0:
            raise FileNotFoundError(
                "relevant .inp and .out file is not found. Try to find out the file and put it in this document."
            )
        else:
            opath = os.getcwd()
            os.chdir(self.loc)
