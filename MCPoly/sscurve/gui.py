...
def gui():
    ...
    def multiple_gui(
        polymers, loc="./", savedata=False, savefig=False, pngname="", csvname=""
    ):
        opath = os.getcwd()
        os.chdir(loc)
        try:
        ...
        for polymer in polymers:
            ...
            try:
                ...
                continue
            ax = plt.plot(
                datum["Strain Length(%)"][:-1], datum["Stress Force(nN)"][:-1], "*-"
            )
            datum = datum.rename(
                columns={
            ...
            ot = pd.merge_ordered(
                ot, datum[:-1], fill_method="ffill", left_by="Stress Force(nN)"
            )
    ...
    print(
        "TIPS: Multiple values can be selected with Shift and/or Ctrl (or Command) pressed and mouse clicks or arrow keys."
    )
    ...
    def geoshow(aim1, aim2, geoi, files):
        ...
        with output2:
            ...
            try:
                view3dchoose(
                    files[geoi] + "_0.000", loc.value, choose=aim, width=450, height=400
                )
            except:
                view3dchoose(
                    files[geoi] + "_0", loc.value, choose=aim, width=450, height=400
                )
    ...
    resultshow = widgets.Button(
        description="Show Chart and table",
    ...
    aim1 = widgets.BoundedIntText(
        description="Atom 1:", value=0, min=0, layout=widgets.Layout(width="48%")
    )
    aim2 = widgets.BoundedIntText(
        description="Atom 2:", value=0, min=0, layout=widgets.Layout(width="48%")
    )
    OK = widgets.Button(description="OK")
    geoi = widgets.IntText(value=0)
    ...
    judgeoutput.layout = widgets.Layout(
        display="flex",
        flew_flow="column",
        overflow="scroll hidden",
        align_items="stretch",
    ...
    geoout = widgets.interactive_output(
        geoshow, {"aim1": aim1, "aim2": aim2, "geoi": geoi, "files": files}
    )
