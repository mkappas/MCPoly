...
def ssgui():
        ...
        with fromoutput:
            for i, polymer in enumerate(polymers):
                print(
                    polymer
                    + ": {0:.6f} Hartree".format(
                        status(polymer, loc).status(figureonly=True)[-1]
                    )
                )
                if fromshow == False:
                    ...
                    view3dchoose(
                        polymer, loc, choose=[aim1, aim2], width=450, height=400
                    )
        os.chdir(opath)
    ...
    def geoshow(file, loc, aim1, aim2, method, bs):
        ...
        with output:
            try:
                ...
                if file == "":
                    print(
                        "Please input the file to show the geometry structure on this screen."
                    )
                else:
                    print("No such file.")
            if method != "" and bs != "":
                display(
                    widgets.HBox([widgets.Label("ORCA Location:"), orcaloc]), startorca
                )
    ...
    def intro_status3d(button):
        ...
        with output2:
            display(
                widgets.Label(
                    "File: {0}.xyz".format(file.value), style=dict(font_weight="bold")
                )
            )
        ...
        if addox1.value == True:
            ...
            if addforce3.value == 0:
                raise ValueError("You can't set the force add per step zero!")
            forcestep = {
                strain.value: addforce.value,
                strain2.value: addforce2.value,
        ...
        while 1:
            ...
            orca(
                file.value + "_{0:.3f}".format(strain.value),
                orcaloc.value,
                loc.value,
                loc.value,
            )
            s = status(file.value + "_{0:.3f}".format(strain.value), loc.value).status(
                statusonly=True
            )
            ...
            if s == 4:
                xoy = checkbroken(
                    file.value, strain.value, forcestep[key], [aim1.value, aim2.value]
                )
                if xoy == 1:
                    with output2:
                        display(
                    ...
                    with fromoutput:
                        for i, poly in enumerate(polys):
                            print(
                                poly
                                + ": {0:.6f} Hartree".format(
                                    status(poly, loc.value).status(figureonly=True)[-1]
                                )
                            )
                            if fromshow == False:
            ...
            for key in keys:
                if strain.value >= key:
                    XYZtoINP(
                        file.value + "_{0:.3f}".format(strain.value),
                        inpname=file.value
                        + "_{0:.3f}".format(strain.value + forcestep[key]),
                        method=method.value,
                        basis_set=bs.value,
                        opt=True,
                        freq=freq.value,
    ...
    strain = widgets.BoundedFloatText(
        description="Start Force(nN):",
        value=0.000,
        min=0.000,
        step=0.100,
        style={"description_width": "initial"},
    )
    addforce = widgets.BoundedFloatText(
        description="Force Add per Step(nN):",
        value=0.100,
        min=0.000,
        step=0.100,
        style={"description_width": "initial"},
    )
    addox1 = widgets.ToggleButton(
        value=False, description="The Second Stage", style=dict(font_weight="bold")
    )
    strain2 = widgets.BoundedFloatText(
        description="Till Force(nN):",
        value=0.000,
        min=0.000,
        step=0.100,
        style={"description_width": "initial"},
    )
    addforce2 = widgets.BoundedFloatText(
        description="Force Add per Step(nN):",
        value=0.100,
        min=0.000,
        step=0.100,
    ...
    addox2 = widgets.ToggleButton(
        value=False, description="The Third Stage", style=dict(font_weight="bold")
    )
    strain3 = widgets.BoundedFloatText(
        description="Till Force(nN):",
        value=0.000,
        min=0.000,
        step=0.100,
    ...
    addforce3 = widgets.BoundedFloatText(
        description="Force Add per Step(nN):",
        value=0.100,
        min=0.000,
        step=0.100,
    ...
    curve = widgets.ToggleButton(
        description="Stress-Strain Curve",
        layout=widgets.Layout(width="99%"),
        style=dict(font_weight="bold", text_decoration="underline"),
    )
    fromshow = widgets.ToggleButton(
        description="Show the site of Force",
        value=True,
        layout=widgets.Layout(width="99%"),
        style=dict(font_weight="bold", text_decoration="underline"),
    )
    savechart = widgets.Button(
        description="Save chart ( .png)", layout=widgets.Layout(width="50%")
    )
    savedata = widgets.Button(
        description="Save data (.txt)", layout=widgets.Layout(width="47.5%")
    )
    ...
    ssout = widgets.interactive_output(
        curveshow, {"curve": curve, "file": file, "loc": loc}
    )
    fromout = widgets.interactive_output(
        showfrom,
        {"fromshow": fromshow, "file": file, "loc": loc, "aim1": aim1, "aim2": aim2},
    )
    ...
    curveoutput.layout = widgets.Layout(
        display="flex",
        flew_flow="column",
        overflow="scroll hidden",
        align_items="stretch",
