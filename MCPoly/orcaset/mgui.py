...
def mgui():
    ...
    def intro_status3d(button):
        ...
        for file in files.value:
            ...
            if s == 4:
                ...
                with output2:
                    display(
                        widgets.HBox(
                            [
                                widgets.Valid(description="     " + file, value=True),
            ...
            with output:
                print(
                    "File: {0}.xyz    Energy: {1:.6f} Hartree".format(
                        file, status(file, loc.value).status(figureonly=True)[-1]
                    )
                )
    ...
    opt = widgets.ToggleButton(
        value=False, description="Optimisation", layout=widgets.Layout(width="50%")
    )
    freq = widgets.ToggleButton(
        value=False, description="Frequency", layout=widgets.Layout(width="50%")
    )
    ...
    geoout = widgets.interactive_output(
        geoshow, {"files": files, "loc": loc, "method": method, "bs": bs}
    )
